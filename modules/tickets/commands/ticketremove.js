...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        const targetUser = message.mentions.users.first() || (args && args[0] ? await client.users.fetch(args[0]).catch(() => null) : null);
        if (!targetUser) {
            return message.reply('Please mention a valid member or provide their user ID. Example: `!ticketremove @username`');
        }
        ...
        try {
            ...
            await message.reply(`Successfully removed **${targetUser.username}** from this ticket channel.`);
        } catch (error) {
            console.error('Failed to remove member from ticket channel:', error);
            message.reply('An unexpected error occurred while updating channel permissions.');
        }
    }
};
